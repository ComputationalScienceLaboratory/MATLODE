...
DevPtr<double> bicg_stab(uint32_t maxnit,                    /* max number of iterations */
	...
	cusparseDnVecDescr_t res_cuda = cuda_vec(res);
...
#pragma region Conjugate Gradient Algorithm
	...
	if (res_nrm <= res_tol) {
		// mexPrintf("CUDA BiCG-stab exited early w/ residual norm %e < %e\n", res_nrm, res_tol);
		tmp_vec.free();
		res.free();
		...
		CHECK_CUSPARSE(cusparseDestroyDnVec(res_cuda));
		CHECK_CUSPARSE(cusparseDestroyDnVec(tmp_cuda));
		return sol;
	}

	// TODO: Think about better choices for this
	DevPtr<double> r_hat = new_gpu_buffer<double>(res.size);
	CHECK_CUBLAS(gpu_math.copy(r_hat, res));

	double rho = CHECK_CUBLAS_VARIANT(gpu_math.dot_product(r_hat, res));

	DevPtr<double> p            = new_gpu_buffer<double>(res.size);
	cusparseDnVecDescr_t p_cuda = cuda_vec(p);
	CHECK_CUBLAS(gpu_math.copy(p, res));
	DevPtr<double> v            = new_gpu_buffer<double>(res.size);
	cusparseDnVecDescr_t v_cuda = cuda_vec(v);

	uint32_t current_iter = 0;
	while (current_iter < maxnit) {
		current_iter += 1;
		CHECK_CUSPARSE(gpu_math.sparse_matvec(mat_cuda, p_cuda, v_cuda));
		...
		CHECK_CUSPARSE(gpu_math.sparse_matvec(mat_cuda, res_cuda, tmp_cuda));
		double omega = CHECK_CUBLAS_VARIANT(gpu_math.dot_product(tmp_vec, res)) /
		               CHECK_CUBLAS_VARIANT(gpu_math.dot_product(tmp_vec, tmp_vec));

		CHECK_CUBLAS(gpu_math.axpy(omega, res, sol));
		CHECK_CUBLAS(gpu_math.axpy(-omega, tmp_vec, res));
		res_nrm = CHECK_CUBLAS_VARIANT(gpu_math.norm(res));

		if (res_nrm < res_tol) { break; }

		double rho_prev = rho;
		rho             = CHECK_CUBLAS_VARIANT(gpu_math.dot_product(r_hat, res));
		double beta     = rho / rho_prev * (alpha / omega);

		CHECK_CUBLAS(gpu_math.scale(p, beta));
		CHECK_CUBLAS(gpu_math.axpy(1, res, p));
		CHECK_CUBLAS(gpu_math.axpy(-omega * beta, v, p));
	}
#pragma endregion

	// mexPrintf("CUDA BiCG-stab stopped after %d iterations w/ res norm %e.\n", current_iter, res_nrm);
	...
	CHECK_CUSPARSE(cusparseDestroyDnVec(res_cuda));
	CHECK_CUSPARSE(cusparseDestroyDnVec(tmp_cuda));
	CHECK_CUSPARSE(cusparseDestroyDnVec(v_cuda));
...
void checkArguments(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
	if (nrhs < 2) { mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:nrhs", "Two inputs required"); }

	if (nlhs != 1) { mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:nlhs", "One output required"); }
	...
	if (a_num_dimension != 2) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:a_dims", "Input 1 (A) must be n-by-n dimension (matrix)");
	}

	if (a_dimensions[0] != a_dimensions[1]) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:a_dims", "Input 1 (A) must be square matrix");
	}

	if (!mxIsSparse(prhs[0])) { mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:a_sparse", "Input 1 (A) must be sparse"); }
	...
	if (mxGetNumberOfDimensions(prhs[1]) > 2 ||
		...
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:b_dim", ss.str().c_str());
	}
	...
	if (!mxIsDouble(prhs[1]) || mxIsComplex(prhs[1])) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:b_type", "Input 2 (b) must be noncomplex double");
	}
	...
	if (mxGetNumberOfElements(prhs[2]) != 1) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:tol_dim", "Input 3 (tolerance) must be a scalar");
	}

	if (!mxIsDouble(prhs[2]) || mxIsComplex(prhs[2])) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:tol_type", "Input 3 (tolerance) must be noncomplex double");
	}

	if (mxGetDoubles(prhs[2])[0] <= 0) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:tol_pos", "Input 3 (tolerance) must be positive");
	}
	...
	if (mxGetNumberOfElements(prhs[3]) != 1) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:maxit_dim", "Input 4 (maxit) must be a scalar");
	}

	if (!get_uint(prhs[3]).has_value()) {
		mexErrMsgIdAndTxt("Matlode:cudaBiCG-stab:maxit_type", "Input 4 (maxit) must be convertible to Uint32");
	}
}
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
	...
	DevPtr<double> sol_dev = bicg_stab(maxit, tol, a_dev, b_dev);
