...
%
function [ Tout, Yout, ISTATUS, RSTATUS, Ierr] = EXIM_MRGARK_FWD_Integrator( OdeFunctionOne,...
...
AF = Coefficient.AMatrixF;
AS = Coefficient.AMatrixS;
gamma = Coefficient.Gamma;
ATF = AF.';
ATS = AS.';
BTF = Coefficient.BTransposeF;
BTS = Coefficient.BTransposeS;
BThatF = Coefficient.BTransposeHatF;
BThatS = Coefficient.BTransposeHatS;
...
NumStagesF = numel(BTF);
NumStagesS = numel(BTS);
NumVars = numel(Y);
...
%     case 2: cannot distinguish between tnext and tcurrent
    ...
    KSlowFast = zeros(NumVars, NumStagesS);
    KFast = zeros(NumVars, NumStagesF);
    KSlow = zeros(NumVars, NumStagesS);
    ...
    NewtonMatrix = (speye(NumVars) - gamma * H * OPTIONS.Jacobian(T, Yout(:, TYindex)));
    %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    % Stages
    %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    for lambda = 1 : M
        ...
        Afslambda = Afs(lambda, gamma, M);
        Asflambda = Asf(lambda, gamma, M);
            
        for stage = 1 : NumStagesF
            
            for col = ColumnCounter : NumStagesS               
                ...
                if (Afslambda(stage, col) ~= 0)
                    ...
                    TimeValSlow = T + H * sum(ATS(:, col));
                    index = 1 : (col - 1);
                    %Evaluate slow stage
                    result = Yout(:, TYindex) + H * KSlow(:, index) * ATS(index, col)...
                        + h * KSlowFast(:, col);
                    ...
                    K = zeros(NumVars, 1);
                    NewtonStep = inf;
                    nmsparse = issparse(NewtonMatrix);
                    if OPTIONS.LU
                        if nmsparse
                            [L,U,P,Q,R] = lu(NewtonMatrix);
                        else
                            [L,U,p] = lu(NewtonMatrix,'vector');
                        end
                    end
                    NewtonItrcounter = 0;
                    prevNewtonNorm = inf;
                    
                    while norm(NewtonStep) > OPTIONS.NewtonTol && NewtonItrcounter < OPTIONS.NewtonMaxIt
                       
                        RHS = (K - (OdeFunctionTwo(TimeValSlow, result + gamma * H * K)));
                        if ~OPTIONS.LU
                            NewtonStep = (NewtonMatrix) ...
                                \ RHS;
                        else
                            if nmsparse
                                NewtonStep = Q * (U \ (L \ (P * (R \ RHS))));
                            else
                                NewtonStep = U \ (L \ RHS(p));
                            end
                        end
                        
                        if abs(prevNewtonNorm - norm(NewtonStep)) < sqrt(eps)
                            NewtonMatrix = (speye(NumVars) - gamma * H * OPTIONS.Jacobian(TimeValSlow, result + gamma * H * K));
                            NewtonItrcounter = 0;
                            prevNewtonNorm = inf;
                            disp('Hello fam');
                        else
                            prevNewtonNorm = norm(NewtonStep);
                            K = K - NewtonStep;
                        end
                        NewtonItrcounter = NewtonItrcounter + 1;
                    end
                    
                    %After Newton is done
                    KSlow(:, col) = K;
                    if lambda == 1
            ...
            TimeValFast = T + h * sum(ATF(stage, :)) + ((lambda - 1)/M) * H;
            
            %Solve the current fast stage
            KFast(:, stage) = OdeFunctionOne(TimeValFast, YLambda ...
                + h * KFast(:, 1 : (stage - 1)) * ATF(1 : stage - 1, stage)...
        ...
        YLambda = YLambda + h * KFast * BTF;
        YHatLambda = YHatLambda + h * KFast * BThatF;
    end
    ...
    % Error estimation
    %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    YTemp = YLambda + H * KSlow * BTS;
    YHat = YHatLambda + H * KSlow * BThatS;
    
    YHatSlow = YLambda + H * KSlow * BThatS;
    YHatFast = YHatLambda + H * KSlow * BTS;
