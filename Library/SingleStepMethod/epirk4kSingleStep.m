...
function [y, yerr, ISTATUS] = epirk4kSingleStep(y0, dt, rhsFun, jacFun, ...
    ...
    % Set Arnoldi tolerance to be high
    arnoldiTol = 1e-12;
    ...
    % Need phi1(3/4)
    [~, phi_ks] = phiKs(3/4 * dt * H, M, 1, LH, UH, pH, dt, 3/4);
    phi1_3_4 = phi_ks(:, :, 1);
    % Need phi1(1)
    [~, phi_ks] = phiKs(dt * H, M, 1, LH, UH, pH, dt, 1);
    phi1_1 = phi_ks(:, :, 1);
    % Need phi1(9/16), phi2(9/16) and phi3(9/16)
    [~, phi_ks] = phiKs(9/16 * dt * H, M, 1:3, LH, UH, pH, dt, 9/16);
    phi1_9_16 = phi_ks(:, :, 1);
    phi2_9_16 = phi_ks(:, :, 2);
    phi3_9_16 = phi_ks(:, :, 3);
    ...
    L_s(:, 3) = L_s(:, 3) + a(2, 2) * dt * (diag(ones(M, 1) * p_tilde(2))) * d_2_1;
    ...
    psiTerm = dt * (p(2, 1) * phi1_9_16 + p(2, 2) * phi2_9_16) * d_2_1;
    L_s(:, 4) = L_s(:, 4) + b(2) * psiTerm;
    yerr_R = yerr_R + (b(2) - b_hat(2)) * psiTerm;

    d_3_1 = D_j_1(2, H, M, E_s, L_s, C);

    psiTerm = dt * (p(3, 1) * phi1_9_16 + p(3, 2) * phi2_9_16 + p(3, 3) * phi3_9_16)  * d_3_1;
