...
%
function [y, yerr, ISTATUS] = epirk3WISingleStepNaiveExp(y0, dt, rhsFun, jacFun, ...
...
function [psi, krySteps] = Psi(i, j, A0, v, N, dt, g, p, MatrixFree, NBasisVectors, krySteps, Tol)
    ...
    if g(i, j) ~= 0
        ...
        psiM = g(i,j)*dt*ones(N, 1);
        psiM = exp(psiM);                           % phi_0

        for k = 1:j
           psiM = (psiM - 1/factorial(k - 1))/(g(i,j) * dt);    % phi_k
...
function r = R(rhsFun, y, y0, A0, MatrixFree)
    ...
    % Assuming A0 is Identity, previously A0 multiplied (y-y0)
    if (~MatrixFree)
        r = rhsFun(y) - rhsFun(y0) - (y - y0);         % Need to find if
                                                            % A0 has to be
                                                            % the actual
                                                            % Jacobian or
                                                            % Krylov subspace
