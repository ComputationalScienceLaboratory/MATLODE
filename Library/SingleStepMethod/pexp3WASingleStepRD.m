function [y, yerr, ISTATUS] = pexp3WASingleStepRD(y0, dt, rhsFun1, rhsFun2, ...
    ...
    for i = 1 : max(s(:))
        ...
        for j = 1:ps
            ...
            if i <= s(j)
                ...
                if j == 1 % diffusion
                    NP    = N/NReactants;
                    K_pi  = zeros(NP, NReactants); 
                    v0_pi = zeros(NP, NReactants);
                    J0_pi = cell(NReactants,1);
                    for pi = 1:NReactants
                       if ~MatrixFree
                            J0_pi{pi} = JacJ{j}((pi - 1) * NP + 1: pi * NP, (pi - 1) * NP + 1: pi * NP);
                        else
                            error('Not implemented yet');
                       end 
                        
                       v0_pi(:, pi) = K_s((pi - 1) * NP + 1: pi * NP, i , j);
                    end
                    
                    parfor pi = 1:NReactants
                        % Compute the Krylov basis matrices for column the K's
                        [V, H, M] = ArnoldiAdapt(J0_pi{pi}, v0_pi(:, pi), NP, dt * G{j,j}(i,i), MatrixFree, ...
                        ...
                        % Compute Phi_1
                        K_pi(:, pi) = Phi_1(G{j, j}(i, i), K_s((pi - 1) * NP + 1: pi * NP, i , j), dt, V, H, M);
                    end
                    
                    for pi = 1:NReactants
                        K_s((pi - 1) * NP + 1: pi * NP, i , j) = K_pi(:, pi);
                    end
                else
                    % Compute the Krylov basis matrices for column the K's
