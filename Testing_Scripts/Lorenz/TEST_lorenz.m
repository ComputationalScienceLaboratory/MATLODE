...
Ode_Hess_vec        = @lorenz_Hess_vec;
Ode_Hesstr_vec      = @lorenz_Hesstr_vec;
Ode_Jacp            = @lorenz_Jacp;
Ode_Hesstr_vec_f_py = @lorenz_Hesstr_vec_f_py;
Ode_QFun            = @lorenz_QFun;
Ode_Mu              = @lorenz_Mu;
Ode_Quadrature      = @lorenz_Quadrature;
Ode_DRDP            = @lorenz_DRDP;
Ode_DRDY            = @lorenz_DRDY;
Ode_Hesstr_vec_r_py = @lorenz_Hesstr_vec_r_py;
Ode_Hesstr_vec_r    = @lorenz_Hesstr_vec_r;
...
enableSave = true;
resultsPath = '/home/adaug/Git_Repository/MatlODE/Testing_Scripts/Lorenz/Lorenz_Results';
...
AbsTol = [ 1e-8; 1e-8; 1e-8 ];
RelTol = [ 1e-8; 1e-8; 1e-8 ];
...
switch ( analysis )
    case -1 % solution
        ...
        if ( Ierr(:) >= 0.0 )
            saveVariable([resultsPath, '/Lorenz_Results_', implementation, ...
                '/Lorenz_Results_', implementation, '_', family, '/', Coefficient.Name, '/'], ...
                ['Solution_' family '_' implementation '_L_T'], T, ...            
               ['Solution_', family, '_', implementation, '_L_Y'], Y, ...
               ['Solution_' family '_' implementation '_L_ISTATUS'], ISTATUS, ...
               ['Solution_' family '_' implementation '_L_RSTATUS'], RSTATUS, ...
               ['Solution_' family '_' implementation '_L_Ierr'], Ierr, enableSave );
            

            titleName = {['Solution: ' family ' ' implementation ' Integrator (Lorenz)'], ...
            ...
            title(titleName);
            if (enableSave == true )
                saveas(gcf, [resultsPath, '/Lorenz_Results_', implementation, ...
                    '/Lorenz_Results_', implementation, '_', family, '/', Coefficient.Name, ...
                    '/Solution_', family, '_', implementation,'_L_', Coefficient.Name, '.fig']);
            end
        end
...
%                elapsedTime_SDIRK_FWD,error_SDIRK_FWD, '-^');
        loglog(elapsedTime_ode45,error_ode15s,'--o', ...
               elapsedTime_ERK_FWD_1,error_ERK_FWD_1, '-+', ...
               elapsedTime_ERK_FWD_4,error_ERK_FWD_4, '-*', ...
               elapsedTime_ERK_FWD_5,error_ERK_FWD_5, '-^', ...
...
%                'MatlODE\_ERK\_FWD (Dopri853)');
        ...
        xlabel('CPU Time (sec)');
        ylabel('Relative Error');
        title('CPU vs. Relative Error (Lorenz3)');
...
%         saveas(gcf,strcat(resultsPath,'/Lorenz3_Explicit_CpuResults.fig'),'fig');
        saveas(gcf,strcat(resultsPath,'/Lorenz3_Implicit_CpuResults.eps'),'epsc');
        saveas(gcf,strcat(resultsPath,'/Lorenz3_Implicit_CpuResults.fig'),'fig');
...
%                nsteps_SDIRK_FWD,error_SDIRK_FWD, '-^');
        loglog(nsteps_ode15s,error_ode15s,'--o', ...
               nsteps_ERK_FWD_1,error_ERK_FWD_1, '-+', ...
               nsteps_ERK_FWD_4,error_ERK_FWD_4, '-*', ...
               nsteps_ERK_FWD_5,error_ERK_FWD_5, '-^', ...
...
%                'MatlODE\_ERK\_FWD (Dopri853)');
        ...
        saveas(gcf,strcat(resultsPath,'/Lorenz3_Implicit_NstepsResults.eps'),'epsc');           
        saveas(gcf,strcat(resultsPath,'/Lorenz3_Implicit_NstepsResults.fig'),'fig');
