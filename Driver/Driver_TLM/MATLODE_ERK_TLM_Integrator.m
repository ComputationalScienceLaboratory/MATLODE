...
%
% |MATLODE_ERK_TLM_Integrator| displays the available methods
% associated with the exponential forward integrator.
%
% |[T, Y, Sens] = MATLODE_ERK_TLM_Integrator(Ode_Function, Time_Interval, Y0,
% Options)| computes the ODE solution with respect to the user supplied
% options configuration and tangent linear sensitivity.
%
% |[T, Y, Sens, Quad, Stats]| = MATLODE_ERK_TLM_Integrator(Ode_Function,
...
% Sensitivity Analysis -> MATLODE_Example_ERK_TLM_Integrator.
%
%% Contact Information
%%
...
%
function [ Tout_TLM, Yout_TLM, Y_TLM, Stats_TLM ] = MATLODE_ERK_TLM_Integrator( OdeFunction, Tspan, Y0, OPTIONS_U )   
    % Display input/output parameters
    if ( nargout == 0 && nargin == 0 )
        fprintf('Syntax: \n');
        fprintf( '                           MATLODE_ERK_TLM_Integrator \n' );
        fprintf( '          [ T, Y, Sens ] = MATLODE_ERK_TLM_Integrator(Ode_Function, Time_Interval, Y0, Options) \n' );
        fprintf( '   [ T, Y, Sens, Stats ] = MATLODE_ERK_TLM_Integrator(Ode_Function, Time_Interval, Y0, Options) \n' );
        fprintf( '\n' );
        return;
    end
    ...
    for interval=1:tspanMaxSize-1
        ...
        [ Tout_TLM_interval, Yout_TLM_interval, Y_TLM_Interval, ISTATUS_TLM_interval, RSTATUS_TLM, Ierr ] = ...
            ERK_TLM_Integrator( OdeFunction,[Tspan(interval), Tspan(interval+1)], Yout_TLM_interval(end,:), OPTIONS, Coefficient);
