...
%
%% Description
% Driver file to solve the system y' = F(t,y) using a Rosenbrock (ROS)
% method.
%
% |MATLODE_ROS_FWD_Integrator| displays the available methods
% associated with the rosenbrock forward integrator.
%
...
% settings. We note that a Jacobian is required for an rosenbrock method.
%
...
% Forward Integration -> MATLODE_Example_ROS_FWD_Integrator.
%
%% Contact Information
%%
...
%
function  [ Tout_FWD, Yout_FWD, Stats_FWD ] = MATLODE_ROS_FWD_Integrator( OdeFunction, Tspan, Y0, OPTIONS_U )
    % Display input/output parameters
    if ( nargout == 0 && nargin == 0 )
        fprintf('Syntax: \n');
        fprintf( '                     MATLODE_ROS_FWD_Integrator \n' );
        fprintf( '          [ T, Y ] = MATLODE_ROS_FWD_Integrator(Ode_Function, Time_Interval, Y0, Options) \n' );
        fprintf( '   [ T, Y, Stats ] = MATLODE_ROS_FWD_Integrator(Ode_Function, Time_Interval, Y0, Options) \n' );
        fprintf( '\n' );
        return;        
    end
    ...
    % Configure Options
    [ OPTIONS, Coefficient ] = OPTIONS_Configuration(OPTIONS_U,'ROS','FWD',Y0, Tspan);
