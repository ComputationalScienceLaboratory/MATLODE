...
function  [ Tout_FWD, Yout_FWD, Stats_FWD ] = MATLODE_ROK_FWD_Integrator( OdeFunction, Tspan, Y0, OPTIONS_U )
    ...
    if ( nargout == 0 && nargin == 0 )
        fprintf('Syntax: \n');
        fprintf( '                     MATLODE_ROK_FWD_Integrator \n' );
        fprintf( '          [ T, Y ] = MATLODE_ROK_FWD_Integrator(Ode_Function, Time_Interval, Y0, Options) \n' );
        fprintf( '   [ T, Y, Stats ] = MATLODE_ROK_FWD_Integrator(Ode_Function, Time_Interval, Y0, Options) \n' );
        fprintf( '\n' );
        return;
    end
    ...
    % Configure Options
    [ OPTIONS, Coefficient ] = OPTIONS_Configuration(OPTIONS_U,'ROK','FWD',Y0, Tspan);
    ...
    for interval=1:tspanMaxSize-1
        ...
        [ Tout_FWD_interval, Yout_FWD_interval, ISTATUS_FWD_interval, RSTATUS_FWD, Ierr ] = ...
            ROK_FWD_Integrator( OdeFunction,[Tspan(interval), Tspan(interval+1)], Yout_FWD_interval(end,:), OPTIONS, Coefficient, adjStackFlag, adjQuadFlag );
