...
%% Syntax
%                   MATLODE_EXP_FWD_Integrator
%          [T, Y] = MATLODE_EXP_FWD_Integrator(Ode_Function, Time_Interval, Y0)
%   [T, Y, Stats] = MATLODE_EXP_FWD_Integrator(Ode_Function, Time_Interval, Y0)
%
%% Input Parameters
...
%
%% Description
% Driver file to solve the system y' = F(t,y) using a Exponential (EXP)
% method.
%
% |MATLODE_EXP_FWD_Integrator| displays the available methods
% associated with the exponential forward integrator.
%
% |[T, Y] = MATLODE_EXP_FWD_Integrator(Ode_Function, Time_Interval, Y0, Options)|
...
% Forward Integration -> MATLODE_Example_EXP_FWD_Integrator.
%
%% Contact Information
%%
...
function  [ Tout_FWD, Yout_FWD, Stats_FWD ] = MATLODE_EXP_FWD_Integrator( OdeFunction, Tspan, Y0, OPTIONS_U )
    ...
    if ( nargout == 0 && nargin == 0 )
        fprintf('Syntax: \n');
        fprintf( '                     MATLODE_EXP_FWD_Integrator \n' );
        fprintf( '          [ T, Y ] = MATLODE_EXP_FWD_Integrator(Ode_Function, Time_Interval, Y0, Options) \n' );
        fprintf( '   [ T, Y, Stats ] = MATLODE_EXP_FWD_Integrator(Ode_Function, Time_Interval, Y0, options) \n' );
        fprintf( '\n' );
        return;
    end
    ...
    % Configure Options
    [ OPTIONS, Coefficient ] = OPTIONS_Configuration(OPTIONS_U,'EXP','FWD',Y0, Tspan);
    ...
    for interval=1:tspanMaxSize-1
        ...
        [ Tout_FWD_interval, Yout_FWD_interval, ISTATUS_FWD_interval, RSTATUS_FWD, Ierr ] = ...
            EXP_FWD_Integrator( OdeFunction,[Tspan(interval), Tspan(interval+1)], Yout_FWD_interval(end,:), OPTIONS, Coefficient, adjStackFlag, adjQuadFlag );
...
%       <td>ptranq@vt.edu</td>
%       <td>Implemented EXP and all supporting components</td>
%   </tr>
%   <tr>
%       <td>1/1/2014</td>
